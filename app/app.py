...
def log_start_time_if_needed(mode: str, clock_dt: datetime):
    """
    כתיבת שעת התחלה (HH:MM) לשורה 4 עבור תאריך clock_dt,
    בפעם הראשונה אחרי 05:00 — פעם אחת ביום לכל מצב.
    """
    if clock_dt is None:
        return
    if clock_dt.hour < RESET_HOUR:
...
def start_timer(i):
    ...
    # ✅ כתיבת שעת התחלה לשורה 4 (לא מפיל את ה-start אם יש בעיה)
    try:
...
def stop_timer(i):
    ...
    # ✅ עצירה אמיתית – שומר את הזמן, מאפס נקודת התחלה
    cur.execute("""
        UPDATE timers
        SET running=0,
            elapsed=?,
...
if __name__ == "__main__":
    # על Render אל תדליק debug=True; לוקאלי זה בסדר
